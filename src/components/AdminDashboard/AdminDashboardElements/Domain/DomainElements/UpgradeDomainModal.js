...
import classes from "./UpgradeDomainModal.module.css";

const UpgradeDomainModal = ({ open, setOpen, domainData, setDomainData }) => {
  ...
  const submitUpgradeDomainHandler = async (event) => {
    ...
    if (mailboxNo) {
      var response = await fetch(
        `https://api.inspedium.email/upgrade-subscription?isAdmin=${true}`,
        requestOptions
      );
    } else {
    ...
    if (response.ok) {
      toast.success("Domain successfully upgraded!");
      setDomainData(null);
      setOpen(false);
    } else {
  ...
  return (
    ...
    >
      <Box sx={style}>
        ...
        <form
          onSubmit={submitUpgradeDomainHandler}
